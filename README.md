<h1 align="center">Minecraft Mod/Game Developer</h1>
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:00ffcc,100:3366ff&height=200&section=header&text=Добро%20Пожаловать%20В%20Ад!&fontSize=40&fontColor=ffffff" />
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Role-Lead_Developer-green?style=flat-square&logo=java&logoColor=white" />
  <img src="https://img.shields.io/badge/Focus-Graphics_Optimization-blue?style=flat-square&logo=render&logoColor=white" />
  <img src="https://img.shields.io/badge/Minecraft-1.12.2-orange?style=flat-square&logo=minecraft&logoColor=white" />
</p>

---

<p align="left">
  <img src="https://skillicons.dev/icons?i=java,gradle,idea,github,blender" />
</p>

---

<h2 align="center">Статистика</h2>
<p align="center">
  <img src="github-metrics.svg" alt="Metrics" width="50%" style="border-radius: 15px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);" />
</p>

## Проекты

### Графика и Рендеринг и Физика/Математика
| Проект | Описание |
| :--- | :--- |
| **Exterminatus ** | Более эффективный мод на оптимизацию игры по сравнению с Optifine и Sodium для 1.12.2 |
| **AR-RenderEngine** | Проект реализующий современные технологии рендеринга в Minecraft 1.12.2 |
| **AR-Particles** | Система для эффективного рендеринга сотен тысяч и миллиона частиц одновременно. |
| **AR-Light** | Продвинутое освещение игрового уровня (аналог Unity/UE4/5) внутри Minecraft. |
| **AR-Render+** | Мод на оптимизацию рендера сущностей и брони за счёт Instanced Rendering. |
| **AR-Blend** | Мод на продвинутую систему смешивания цветом при помощи алгоритма WBOIT/MBOIT |
| **Ar-Collision** | Реализация **OBB** (Oriented Bounding Box) системы коллизий. |

### Механики и Системы
| Проект | Описание |
| :--- | :--- |
| **Ar-Vehicle** | Мод на транспортные средства. |
| **Ar-Deco** | Декоративные блоки с поддержкой рендеринга на основе GLTF моделей. |
| **AR-Libs** | Библиотека инструментов и вспомогательного кода для моих модов. |
| **ModelCitizens-Remake** | ремейк мода ModelCitizens для версии 1.12.2. |
| **MWF-Ultimate** | Не зависимый форк мода "Modular Warfare". |
| **KeyBind+** | Мод который Добовляет сохранение биндов клавишь даже если мод был удалён/изменён. |
| **Key-And-Code-Lock-Mod-1.12.2** | Мод с 0 написанный аналог моду Key-And-Code-Lock-Mod для версии 1.7.10. |
| **Ne-Carpenter’s Blocks** | Мод полный аналог Carpenter’s Blocks-1.7.10. |
| **ModularContents** |Мод для создания разного контента в майнкрафт под MIT лицензией (Основной автор проекта Ethan а я там в роле помощника) |
| **Yuushya-Townscape-1.12.2** |порт мода Yuushya-Townscape 1.20.1 на 1.12.2 с улучшением мода |
| **FPV To Minecraft** |порт мода FPV To Minecraft 1.20.1 на 1.12.2 (мод FPV To Minecraft который я портировал является приватным модом с бусти) |

### Приложения и Инструментарий
| Приложение/Инструмент | Описание |
| :--- | :--- |
| **Flans-Model Converter** | Деобфускатор моделей Flans и конвертер в **OBJ/GLB** формат. (Java 21+). |
| **YT-Tools** | Набор инструментов для упрощения создания контента на **YouTube**. |
| **Ar-Vehicle-Pack Editor** | Приложение для создания контента для мода **Ar-Vehicle** . |

---

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:3366ff,100:00ffcc&height=120&section=footer"/>
</p>
